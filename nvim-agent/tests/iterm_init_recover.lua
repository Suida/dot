-- Integration-test init (crash recovery): same socket + agent core bootstrap
-- as iterm_init.lua, but with auto_recover = 'always' so a dirty manifest
-- respawns workers without prompting. The socket block below already deletes
-- any stale .agent/nvim.sock pointer file left by a force-killed server.
